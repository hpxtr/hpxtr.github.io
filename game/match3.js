...
window.onload = function () {
  ...
  var horcruxes = [
    ...
    {type: "cup", value: 0, img: "game/img/cup.png", selected: "", probability: 14},
    {
      type: "diadem",
      value: 0,
  ...
  function drawBoard() {
    ...
    for (var i = 0; i <= level.columns; i++) {
      // vertical
      context.fillRect(level.x + i * level.tilewidth, level.y, borderwidth,
          levelheight);

      //horizontal
      context.fillRect(level.x, level.y + i * level.tileheight, levelwidth,
          borderwidth);
    }
    context.fillRect(level.x, level.y, borderwidth, levelheight);
    context.fillRect(level.x + level.columns * level.tilewidth, level.y,
        borderwidth, levelheight);
    // horizontal
    context.fillRect(level.x, level.y, levelwidth, borderwidth);
    context.fillRect(level.x, level.y + level.rows * level.tileheight,
        levelwidth, borderwidth);

    // scores borders
    // vertical
    context.fillRect(horcrux_x, horcrux_y, borderwidth, horcrux_h);
    context.fillRect(horcrux_x + horcrux_w * horcruxes.length, horcrux_y,
        borderwidth, horcrux_h);
    // horizontal
    context.fillRect(horcrux_x, horcrux_y, horcrux_w * horcruxes.length,
        borderwidth);
    context.fillRect(horcrux_x, horcrux_y + horcrux_h,
  ...
  function drawScores() {
    ...
    for (var i = 0; i < horcruxes.length; i++) {
      ...
      image.onload = context.drawImage(image, horcrux_x + i * horcrux_w,
  ...
  function drawSelectedTile(x, y, type) {
    ...
    image.onload = context.drawImage(image, x + 2, y + 2, level.tilewidth - 4,
        level.tileheight - 4);
  }

  function drawTileWithType(x, y, type_id) {
    var tile = document.getElementById(horcruxes[type_id].type);
    context.drawImage(tile, x + 2, y + 2, level.tilewidth - 4,
        level.tileheight - 4);
  }
  ...
  function getRandomTile() {
    ...
    for (var i = 0; i < horcruxes.length; i++) {
      if (random < level.columns * level.rows * horcruxes[i].probability
          / 100) {
        return i;
      } else {
        random = random - level.columns * level.rows * horcruxes[i].probability
            / 100;
      }
    }
  ...
  function onTouchStart(evt) {
    ...
    debug = "start: " + pos.x + "; " + pos.y + " = " + mt.x + "; " + mt.y + " "
        + mt.valid + ". \n";
    if (mt.valid) {
      start = mt;
    }
  ...
  function onTouchEnd(e) {
    ...
    var mt = getMouseTile(pos);
    debug = debug + "end: " + pos.x + "; " + pos.y + " = " + mt.x + "; " + mt.y
        + " " + mt.valid + ". \n";
    if (mt.valid) {
      if (start) {
  ...
  function onTouchMove(e) {
    var pos = getTouchPos(canvas, e);
    var mt = getMouseTile(pos);
    debug = debug + "move: " + pos.x + "; " + pos.y + " = " + mt.x + "; " + mt.y
        + " " + mt.valid + ". \n";
    if (mt.valid) {
  ...
  function onMouseMove(e) {
    ...
    if (drag && level.selectedtile.selected) {
      ...
      if (mt.valid) {
        ...
        // Check if the tiles can be swapped
        if (canSwap(mt.x, mt.y, level.selectedtile.column,
            level.selectedtile.row)) {
          // Swap the tiles
          mouseSwap(mt.x, mt.y, level.selectedtile.column,
              level.selectedtile.row);
        }
      }
    }
  ...
  function onMouseDown(e) {
    ...
    if (!drag) {
      ...
      if (mt.valid) {
        ...
        var swapped = false;
        if (level.selectedtile.selected) {
          if (mt.x == level.selectedtile.column && mt.y
              == level.selectedtile.row) {
            // Same tile selected, deselect
            level.selectedtile.selected = false;
            drag = true;
            return;
          } else if (canSwap(mt.x, mt.y, level.selectedtile.column,
              level.selectedtile.row)) {
